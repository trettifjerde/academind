const COINS: number[] = [1, 2, 5, 10, 20, 50, 100];
type Info = {total: number, coins: Map<number, number>}

function giveChange(target: number, source: number) : Info {
    const info : Info = {total: 0, coins: new Map()};
...
function giveChangeRecursive(target: number, source: number): Info {
    if (source <= target) {
...
function getNextCoin(amount: number) {
...
const amounts : [number, number] = [326, 500]
